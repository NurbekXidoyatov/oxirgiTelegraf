...
export default function AcceptedMessageFromEmployeesToManagerTable({
  showSeparateMessageModal,
}) {
  const { acceptedmessages } = useSelector(
    (state) => state.acceptedMessagesForManagers
  );
      ...
      <Table.Body items={users}>
        {(item) => (
          ...
          >
            {(columnKey) => (
              <Table.Cell>
                <Tooltip onClick={() => showSeparateMessageModal(item.id)}>
