import React, { useEffect, useState } from "react";
import { useHistory } from "react-router-dom";
import { useHttp } from "../AsyncURL/useHttp";
import { Badge } from "@nextui-org/react";
import { UniversalURL } from "../AsyncURL/BaseUrl";
import Overlay from "../Xodimlar/Overlay";
import "../Employees/employees.css";

export default function SeparateMessagesManager({
  singleId,
  setShowSeparateMessage,
  showSeparateMessage,
  chooseAccept,
  chooseCancel,
  setshowAcceptbtn,
  showAcceptbtn,
}) {
  const { request } = useHttp();
  const history = useHistory();
  const [message, setMessage] = useState([]);

  useEffect(() => {
    request(`${UniversalURL}accepted/message/view/${singleId}`, "GET", null, {
      Authorization: `Bearer ${localStorage.getItem("token")}`,
      "Content-type": "application/json",
    })
      .then((response) => {
        setMessage(response?.data);
        setshowAcceptbtn(showAcceptbtn);
      })
      .catch((error) => console.log(error));
  }, [showAcceptbtn]);

  const backgroundImage1 =
    message?.fileType === "JPG" ||
    message?.fileType === "jpg" ||
    message?.fileType === "png" ||
    message?.fileType === "PNG"
      ? `url(${UniversalURL}file/file-priview/${message?.hashId})`
      : `url(data:image/png;base64,)`;
  return (
    <div>
      <Overlay />
      <div className="blog-card">
        <div className="meta">
          <div
            className="photo"
            style={{
              backgroundImage: backgroundImage1,
              backgroundSize: "cover",
            }}
          ></div>
          <ul className="details">
            <li className="author">
              <a href="##">{message?.fromUser?.fullName}</a>
            </li>
            <li className="date">{message?.createdAt}</li>
            <li className="tags">
              {message?.hashId !== null ? (
                <a href={`${UniversalURL}file/download/${message?.hashId}`}>
                  <strong>Download File</strong>
                </a>
              ) : (
                <p style={{ color: "blue" }}>
                  <strong>File Biriktirilmagan</strong>
                </p>
              )}
            </li>
          </ul>
        </div>
        <div className="description">
          <h1>{message?.title}</h1>
          <p>{message?.context}</p>
          <p className="read-more">
            {message?.messageStatus === "SENDING" ||
            message?.messageStatus === "READING" ? (
              <Badge css={{cursor:"pointer"}} color="error" variant="bordered" onClick={() => chooseCancel(singleId)}>
                Rad etish
              </Badge>
            ) : null}

            {message?.messageStatus === "SENDING" ||
            message?.messageStatus === "READING" ? (
              <Badge css={{cursor:"pointer"}} color="success" variant="bordered" onClick={() => chooseAccept(singleId)}>Qabul qilish</Badge>
            ) : null}

            {message?.messageStatus === "ACCEPTED" ? (
              <Badge css={{cursor:"pointer"}} color="success" variant="bordered" onClick={() => history.push(`/forwardMessage/${message.id}`)}>
                Xodimlarga jo'natish
              </Badge>
            ) : null}
            {message?.messageStatus === "ACCEPTED" ? (
              <Badge
                color="success" variant="bordered"
                css={{cursor:"pointer"}}
                onClick={() =>
                  history.push(`/forwardMessageManagers/${message.id}`)
                }
              >
                Telegraflarga jo'natish
              </Badge>
            ) : null}

            {message?.messageStatus === "CANCELED" ? (
              <Badge css={{cursor:"pointer"}} color="error" variant="bordered" onClick={() => console.log(singleId)}>Rad etildi</Badge>
            ) : null}

            <Badge css={{cursor:"pointer"}} color="success" variant="bordered" onClick={() => setShowSeparateMessage(!showSeparateMessage)}>
              Go back
            </Badge>
          </p>
        </div>
      </div>
    </div>
  );
}
