...
export default function ForwardToEmployees({ showBasket, page, getAllMessages}) {
  ...
  console.log(xabarFayli);


  useEffect(() => {
    request(`${UniversalURL}accepted/message/view/${id}`, "GET", null, {
  ...
  useEffect(() =>{
    if (message) {
      setxabarSarlavhasi(message.title);
      setxabarMatni(message.context);
    }
  },[message])
  ...
  let optionsEmployee = employees.map((item) => ({
    label: item.fullName.split(" ")[0] + " " + item.fullName.split(" ")[1].substring(0,1) + ".  -  " + item.jobName,
    value: item.id,
  }));
