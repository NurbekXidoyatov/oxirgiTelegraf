import React, { useEffect, useState } from "react";
import { useParams, useHistory } from "react-router-dom";
import { useSelector, useDispatch } from "react-redux";
import { useHttp } from "../AsyncURL/useHttp";
import { infoOfSeparateMessage } from "./employeesSlice";
import { UniversalURL } from "../AsyncURL/BaseUrl";
import { Textarea } from "@nextui-org/react";
import "./employees.css";
import Overlay from "../Xodimlar/Overlay";
import ForvardToEmployeesFromEmployees from "../forwardMessage/ForvardToEmployeesFromEmployees";

export default function SeparateMessage() {
  const { id } = useParams();
  const { goBack } = useHistory();
  const { request } = useHttp();
  const dispatch = useDispatch();
  const { separateMsg } = useSelector((state) => state.employeesprofile);
  const [showForvardModal, setShowForvardModal] = useState(false)
  

  useEffect(() => {
    request(`${UniversalURL}emp/message/view/${id}`, "GET", null, {
      Authorization: `Bearer ${localStorage.getItem("token")}`,
      "Content-type": "application/json",
    })
      .then((response) => {
        dispatch(infoOfSeparateMessage(response.data));
      })
      .catch((error) => console.log(error));
  }, []);

  const backgroundImage1 = separateMsg.fileType === "JPG" || separateMsg.fileType === "jpg" || separateMsg.fileType === "png" || separateMsg.fileType === "PNG" ?  `url(${UniversalURL}file/file-priview/${separateMsg.hashId})` :  `url(data:image/png;base64,)`

  return (
    <div>
      <Overlay />
      {showForvardModal ? <ForvardToEmployeesFromEmployees showForvardModal={showForvardModal} setShowForvardModal={setShowForvardModal} separateMsg={separateMsg}/> : null}
      <div className="blog-card">
        <div className="meta">
          <div className="photo" style={{backgroundImage:backgroundImage1, backgroundSize:"cover"}}></div>
          <ul className="details">
            <li className="author">
              <a href="##">{separateMsg?.fullName}</a>
            </li>
            <li className="date">{separateMsg?.createdAt}</li>
            <li className="tags"> {separateMsg?.hashId !== null ? (
              <a
                href={`${UniversalURL}file/download/${separateMsg?.hashId}`}
              >
                <strong>Download File</strong>
              </a>
            ) : (
              <p style={{ color: "blue" }}>
                <strong>File Biriktirilmagan</strong>
              </p>
            )}</li>
          </ul>
        </div>
        <div className="description">
          <h1 className="mb-3">{separateMsg?.title}</h1>
          <Textarea initialValue={separateMsg?.context}  rows={10}/>
          <div className="read-more">
            <p  onClick={goBack}>
              Go back
            </p>
            <p  onClick={() => setShowForvardModal(!showForvardModal)}>
              Xodimlarga yuborish
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}
